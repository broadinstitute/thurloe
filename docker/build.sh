...
function docker_cmd()
{
    if [ $DOCKER_CMD = "build" ] || [ $DOCKER_CMD = "push" ]; then
        ...
        docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/trivy-cache:/root/.cache/ aquasec/trivy --severity CRITICAL --no-progress --ignore-unfixed "$DOCKERHUB_REGISTRY":"${HASH_TAG}"
