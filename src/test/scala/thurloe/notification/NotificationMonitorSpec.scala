...
class NotificationMonitorSpec(_system: ActorSystem) extends TestKit(_system) with FlatSpecLike with Matchers with BeforeAndAfterAll {
  ...
  "NotificationMonitor" should "send notifications" in {
    ...
    val testNotifications = (for (i <- 0 until workerCount * 4) yield WorkspaceInvitedNotification(RawlsUserEmail(s"foo$i"), RawlsUserSubjectId(s"bar$i"), WorkspaceName("namespace", "name"), "some-bucket-name"))
  ...
  it should "maybe send notifications" in {
    ...
    val removedNotification = WorkspaceRemovedNotification(RawlsUserSubjectId(userId), "foo", workspaceName, RawlsUserSubjectId("a_user_id2"))
    val addedNotification = WorkspaceAddedNotification(RawlsUserSubjectId(userId), "foo", workspaceName, RawlsUserSubjectId("a_user_id2"))
  ...
  it should "ignore SendGrid notifications when the monitor is unable to send them due to missing key/value pairs" in {
    ...
    Await.result(ThurloeDatabaseConnector.set(UserKeyValuePairs(userId, Seq(KeyValuePair(NotificationMonitor.notificationsOffKey, "false")))), Duration.Inf)
